...
export function getAllLogsFromDB(): ActivityLog[] {
  ...
  if (!stored || stored === '[]') {
    // Initialize with default logs if empty
  ...
  try {
    ...
    return parsed.length > 0 ? parsed : DEFAULT_LOGS;
  } catch {
    return DEFAULT_LOGS;
  }
}
